...
int main(int argc, char** argv) {
    ...
    try {
        ...
        /*
         * cf->Get函数返回的是字符串："tcp://localhost:9988
         * Communicator::Get函数返回一个new TcpCommunicator(communicator)对象
         */
        Communicator *c = Communicator::Get(cf->Get("communicator"));
        /*
         * cf->Get返回值为字符串"cudart"
         * 执行后plugins中只有一个值：cudart
         */
        vector<string> plugins = split(cf->Get("plugins"), ",");
        Backend b(plugins);//构造函数把plugins复制给Backend的数据成员mPlugins
        b.Start(c);//无限循环，等待前端的连接申请
        delete c;
    } catch (string &e) {
