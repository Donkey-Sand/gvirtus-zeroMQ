...
bool CudaRtHandler::CanExecute(std::string routine) {
    ...
    //mspHandlers是在Initialize（）中加入内容的
...
/*
 * 这个函数的返回值类型为Result指针，将被用来反馈给前端
 */
Result * CudaRtHandler::Execute(std::string routine, Buffer * input_buffer) {
    map<string, CudaRtHandler::CudaRoutineHandler>::iterator it;	//第二个参数是函数指针，指向某个函数
    it = mspHandlers->find(routine);
#ifdef DEBUG
    ...
    return it->second(this, input_buffer); //it->second是一个函数指针，用来调用对用的函数，如CUDA_ROUTINE_HANDLER(GetDeviceCount)等
}
